...
def get_overseas_future_time_price(SRS="6AZ23", EXCH="CME", STRT_DT="", END_DT="", CNT="100", GAP="1", dataframe=None,
                                   file_name=None):  # 해외선물옵션시세 > 해외선물 분봉조회
    """
    ...
    if not dataframe.empty and dataframe.iloc[-1, 0] == current_dataframe.iloc[-1, 0] and dataframe.iloc[-1, 1] == \
            current_dataframe.iloc[-1, 1]:
        ...
        if STRT_DT == END_DT:
            ...
            dataframe['datetime'] = pd.to_datetime(
                dataframe['data_date'].astype(str) + dataframe['data_time'].astype(str).str.zfill(6), errors='coerce',
                format='%Y%m%d%H%M%S')
...
# 해외선물분봉조회 호출
print("Downloading...")
result_dataframe = get_overseas_future_time_price(SRS=SRS, EXCH=EXCH, STRT_DT=STRT_DT, END_DT=END_DT, CNT="100",
                                                  GAP=GAP)
result_dataframe[:3]
